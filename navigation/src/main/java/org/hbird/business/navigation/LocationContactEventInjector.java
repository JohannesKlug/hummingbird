...
public class LocationContactEventInjector extends ElevationDetector {
	 ...
	 */
	public LocationContactEventInjector(double maxCheck, double elevation, TopocentricFrame topo, Satellite satellite, Location location) {
	...
	public int eventOccurred(final SpacecraftState s, final boolean increasing) throws OrekitException {
		...
		exchange.getIn().setBody(new LocationContactEvent("Visibility", "", s.getDate().toDate(TimeScalesFactory.getUTC()).getTime(), location, satellite));
