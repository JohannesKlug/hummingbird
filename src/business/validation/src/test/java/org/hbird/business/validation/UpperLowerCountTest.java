...
public class UpperLowerCountTest extends AbstractJUnit38SpringContextTests {
	...
	protected void sendParameter(final ProducerTemplate template, final Parameter parameter) {
		exchange = new DefaultExchange(context);
		exchange.getIn().setBody(parameter);
		template.send(exchange);
	}

	// FIXME Change to camel asserts using mock endpoints.
	// FIXME Test is far too large. Cannot tell what is being tested. Error messages are output but they may be expected
	...
	public void testEjectionMessage() throws Exception {
		...
		/**
		 * Both limits enabled (null). Send a few parameters that are all in limit. Validate that no state changes
		 * occur.
		 * 
		 * Two messages will be send; 1) UpperLimit = true 2) LowerLimit = true
		 */
		...
		/**
		 * One messages will be send; 3) UpperLimit = false
		 */
		...
		/**
		 * Violate lower limit. Upper limit will again become true, lower limit will remain true because this is the
		 * first violation.
		 */

		/**
		 * One messages will be send; 4) UpperLimit = true
		 */
		...
		/**
		 * One messages will be send; 5) LowerLimit = false
		 */
		...
		/**
		 * One messages will be send; 6) LowerLimit = true
		 */
		 ...
		 * 
		 * This will immediately trigger the calculation of the limit. The counter will register this, but no message
		 * will be issued.
		 */
		...
		/**
		 * One messages will be send; 7) LowerLimit = false
		 */
		sendParameter(startParameterUpperLimit, new Parameter("Parameter1", "", 30d, ""));
