...
public class RangeCheck extends AbstractCheck {
	...
	public RangeCheck(long executionTime, StateParameter stateParameter, Parameter lowerValue, Parameter upperValue) {
		super(executionTime, stateParameter);
	...
	public boolean validate(Parameter parameter) {
		...
		/** Check states. */
		boolean inLower = Comperator.compare(lowerValue.getValue(), parameter.getValue()) < 0;
		boolean inUpper = Comperator.compare(parameter.getValue(), upperValue.getValue()) < 0;
