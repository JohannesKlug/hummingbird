...
public class SystemMonitoringTest extends AbstractJUnit38SpringContextTests {
	...
	public void testEjectionMessage() throws Exception {		
		...
		assertTrue(((Parameter)resultEndpoint.getReceivedExchanges().get(0).getIn().getBody()).getName().equals("Memory"));

		exchange = new DefaultExchange(context);
		threads.send(exchange);
		assertTrue(resultEndpoint.getReceivedCounter() == 2);
		assertTrue(((Parameter)resultEndpoint.getReceivedExchanges().get(1).getIn().getBody()).getName().equals("Threads"));
