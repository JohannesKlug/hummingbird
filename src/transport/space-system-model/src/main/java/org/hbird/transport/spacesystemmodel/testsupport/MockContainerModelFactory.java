...
public class MockContainerModelFactory implements ContainerFactory {
	...
	private void initialise() throws InvalidParameterTypeException {
		...
		final ContainerImpl tmFrame = new ContainerImpl(TM_FRAME_ALIAS, "Test frame", "Test TM frame for unit testing");
		this.addToContainers(tmFrame);
		final ContainerImpl tmFrameHeader = new ContainerImpl(TM_FRAME_HEADER_ALIAS, "Test header", "Test TM header for unit testing");
		this.addToContainers(tmFrameHeader);
		final ContainerImpl tmPacket = new ContainerImpl(TM_PACKET_ALIAS, "Test packet", "Test TM packet for unit testing");
		this.addToContainers(tmPacket);
		final ContainerImpl tmFrameTail = new ContainerImpl(TM_FRAME_TAIL_ALIAS, "Test tail", "Test TM tail for unit testing");
		...
		final ContainerImpl tmPacketHeader = new ContainerImpl(TM_PACKET_HEADER_ALIAS, "Test packet header", "Test TM packet header for unit testing");
		...
		final ContainerImpl tmPacketBody = new ContainerImpl(TM_PACKET_BODY_ALIAS, "Test packet body", "Test TM packet header for unit testing");
		...
		final ContainerImpl packetTypeA = new ContainerImpl("TMPacketTypeA", "Test packet body", "Test TM packet for unit testing");
		...
		// Create a packet type, add it to the packet body, add a restriction to
		// associate it to an ID, and add it to the containers collection
		final ContainerImpl packetTypeB = new ContainerImpl("TMPacketTypeB", "Test packet body", "Test TM packet for unit testing");
		...
		final List<Container> packetInners = new ArrayList<Container>(2);
	 ...
	 * @param container
	 */
	public void addToContainers(final ContainerImpl container) {
		this.containers.put(container.getName(), container);
	}

	private void addToParameters(final ParameterContainer parameter) {
