...
public class SinusCurveParameter extends BaseParameter {
	 ...
	 * @param name
	 */
	public SinusCurveParameter(double angularFrequency, double amplitude, double phase, double intercept, long modolus, String name) {
		super(name);
	...
	public void process(Exchange arg0) {
		try {
			...
			Double value = amplitude * Math.sin(angularFrequency * ((((new Date()).getTime() - startTime.getTime()))%modolus) + phase) + intercept;
